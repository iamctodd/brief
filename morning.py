#!/usr/bin/env python3
"""Morning Brief - Clean, simple, working version."""
...
def get_client():#!/usr/bin/env python3
"""Morning Brief - Clean, simple, working version."""
...
def generate():
    """Generate briefs for all topics."""
    topics = request.json.get("topics", [])
    if not topics:
        return jsonify({"error": "No topics"}), 400
    
    print(f"\n🔍 Generating briefs for: {topics}")
    articles_by_topic = fetch_articles(topics)
    print(f"📰 Articles fetched: {[(t, len(a)) for t, a in articles_by_topic.items()]}")
    
    global current_briefs
    current_briefs = articles_by_topic
    
    # Generate all briefs now (simple, no streaming)
    briefs_html = ""
    for topic in topics:
        ...
        if articles:
            ...
            """
                ...
                import traceback
                traceback.print_exc()
                briefs_html += f"<p style='color: red;'>Error generating brief for {topic}: {str(e)}</p>"
        else:
...
def send_email():
    ...
    try:
        ...
        message["To"] = email
        # In real version, would use current_briefs to generate HTML
        message.attach(MIMEText("<p>Your brief</p>", "html"))
